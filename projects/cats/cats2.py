...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for i in range(len(valid_words)):
        if typed_word == valid_words[i]:
            return typed_word
        elif diff_function(typed_word, valid_words[i], limit) < lowest:
            lowest = diff_function(typed_word, valid_words[i], limit)
            lowest_index = i

    if lowest <= limit:
        # print('DEBUG', 'I am inside the first if: ', limit, lowest)
...
def feline_flips(start, goal, limit):
    ...
    5
    """
    def helper(s, g, total):
        if total > limit:
            return limit + 1
        if s == '' or g == '':
            return total + abs(len(g)-len(s))
        if s[0] == g[0]:
            return helper(s[1:], g[1:], total)
        else:
            return helper(s[1:], g[1:], total + 1)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    0.2
    """
    # upload progress to the server
    # return progress with ID
    # find progress
    correct = 0
...
def time_per_word(words, times_per_player):
    ...
    for t in times_per_player:
        # print("DEBUG:", "out loop")
        for i in range(len(t)-1):
            times += [abs(t[i]-t[i+1])]
            # print("DEBUG:", times)
        times_list += [times]
        times = []
        # print("DEBUG:", times_list)
    return match(words, times_list)
    # times = []
    # list_of_times = []
    # for elem in times_per_player:
        
    #     print('DEBUG', 'I am a list: ', elem, len(elem))
    #     for index in range(len(elem) - 1):
    #         times += elem[index + 1] - elem[index] 
    #     list_of_times += [lst]
    #     lst = []
    # return match(words, list_of_times)
...
def fastest_words(match):
    ...
    #     fastest = min([t[w] for t in get_times(match)])
    #     ranking += [ranking[get_words(w)[w]]]
    # return ranking
    ranking = []
    for p in player_indices: # create an empty list of lists for each player
        ranking += [[]]
    # print("DEBUG:", ranking)
    ...
    for w in word_indices:
        ...
        # for p in player_indices:
            word_time_all_players = [time(match, p, w) for p in player_indices]# time for player p for word w
            print("DEBUG:", word_time_all_players)
            winner = word_time_all_players.index(min(word_time_all_players)) # index of player with min time
            # print("DEBUG:", y)
            ranking[winner].append(word_at(match, w))
    return ranking
