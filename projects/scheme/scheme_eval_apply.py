...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        parent_frame = env.make_child_frame(procedure.formals, args)
        return eval_all(procedure.body, parent_frame)
    else:
...
def eval_all(expressions, env):
    ...
    while expressions:
        if expressions.rest is nil:
            first, expressions = scheme_eval(expressions.first, env, True), nil
        else:
            first, expressions = scheme_eval(expressions.first, env), expressions.rest
    return first
