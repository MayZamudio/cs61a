...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        func_name = do_lambda_form(Pair(signature.rest, expressions.rest), env)
        env.define(signature.first, func_name)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        name, value = bindings.first.first, bindings.first.rest
        names = Pair(name ,names)
        values = Pair(eval_all(value, env), values)
