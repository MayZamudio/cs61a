...
class Bee(Insect):
    ...
    def slow(self, length):
        ...
        print("DEBUG:", self.slow_turns_remaining)
        # END Problem EC
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            print("DEBUG: Found my target")
            target.slow(3)

    def action(self, gamestate):
        """Throw a leaf at the nearest Bee in range."""
        print("DEBUG: Found my target")
        self.throw_at(super().nearest_bee())
        gamestate.time +=1
