...
class Ant(Insect):
    ...
    def add_to(self, place):
        """Places a container ant in place that can contain one non-container ant."""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if place.bees and self.min_range <= index <= self.max_range:
...
class HungryAnt(Ant):
    """The HungryAnt eats a random bee in its place. After eating a Bee, HungryAnt must 
        spend 3 turns chewing before eating again.
    """
    ...
    def action(self, gamestate):
        """The HungryAnt waits three turns after eating a bee to eat another bee."""
        if self.chew_countdown != 0: # chewing   
            self.chew_countdown -= 1
        elif self.place.bees:
            bee_to_eat = random_bee(self.place.bees)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        """Return TRUE if can contain another Ant"""
        if self.ant_contained is None and not other.is_container:
            return True

    def store_ant(self, ant):
        """Stores the ANT in the container"""
    ...
    def action(self, gamestate):
        """Takes the action of the contained ant."""
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, health=2):
        """Starts the HEALTH at 2."""
        super().__init__(health)

class TankAnt(ContainerAnt):
    """ TankAnt deals a damage of 1 to all bees in its place. The contained
        ant deals its own damage.
    """
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is waterproof ant."""
...
class QueenAnt(ScubaThrower):
    ...
    def construct(cls, gamestate):
        ...
        """
        if not gamestate.has_queen and gamestate.food >= cls.food_cost:
            gamestate.has_queen = True
            gamestate.food -= cls.food_cost
    ...
    def remove_from(self, place):
        """"The queen ant cannot be removed"""
        return
...
class Bee(Insect):
    ...
    damage = 1
    is_waterproof = True
    slow_turns_remaining = 0
    scared_turns_ramaining = 0
    is_scared = False
    ...
    def action(self, gamestate):
        ...
        back_destination = self.place.entrance
       
        if self.blocked():
            self.sting(self.place.ant)
        elif self.health > 0 and destination is not None:
            if gamestate.time % 2 == 1 and self.slow_turns_remaining > 0:
                pass
            else:
                if self.scared_turns_ramaining > 0:
                    if self.place.is_hive:
                        self.scared_turns_ramaining -= 1
                    else:
                        self.move_to(back_destination)
                        self.scared_turns_ramaining -= 1
                else:
                    self.move_to(destination)
                    
        if self.slow_turns_remaining > 0:
            self.slow_turns_remaining -= 1
    ...
    def slow(self, length):
        ...
        self.slow_turns_remaining += length
    ...
    def scare(self, length):
        ...
        if not self.is_scared:
            self.scared_turns_ramaining += length
            self.is_scared = True
