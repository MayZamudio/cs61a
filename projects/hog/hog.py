...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while current_roll <= num_rolls:
        ...
        if roll_val == 1:
            ...
            turn_score += roll_val  # accumulate turn score
        current_roll += 1
    if so_sad == 1:
        turn_score = 1
...
def picky_piggy(score):
    ...
    """
    # find nth digit of decimal expansion of 1/7 (0.14285714...)
    if score != 0: 
        if score % 6 == 1: 
            return 1
        if score % 6 == 2:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0, score1) < goal:
        ...
        who = next_player(who)
        say = say(score0, score1) # commentary
    ...
    def say(score0, score1):
        if who == 0: # player 0
            if score0 > last_score and score0-last_score > running_high:
                print(str(score0 - last_score) + " point(s)! That's a record gain for Player 0!")
                return announce_highest(who, score0, score0 - last_score) # update running_high
            return announce_highest(who, score0, running_high) # running_high is unchanged
        else: # player 1
            if score1 > last_score and score1-last_score > running_high:
                print(str(score1 - last_score) + " point(s)! That's a record gain for Player 1!")
                return announce_highest(who, score1, score1 - last_score) # update running_high
            return announce_highest(who, score1, running_high) # running_high is unchanged
    return say
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average_it(*args):
        """Return the average value of repeatedly calling the original_function
        """
        k, sum = 1, 0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_average = get_average(1, dice) # assume default max to be for k = 1
    while k <= 10:
        current_average = get_average(k, dice)
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    current_score = picky_piggy(opponent_score)
    if current_score >= cutoff or score + current_score == opponent_score:
        return 0
    else:
        return num_rolls
