...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r'\b(IIV|IX|IV|V?I{1,3}|V)', text)
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'\b((CS|cs|CS\s)\d+[A-Ca-c]{0,})', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'\b([0-2]{0,}[0-9]:[0-5][0-9][AM|PM]{0,})', text)
...
def most_common_code(text):
    ...
    '123'
    """
    counter, codes = 0, area_codes(text)

    for i in codes:
        most_common = codes.count(i)
