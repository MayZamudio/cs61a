...
def pingpong(n):
    ...
    True
    """
    def helper(index, k, direction):
        if n == index:
            return k        
        else:
            if index % 8 == 0 or num_eights(index) > 0:
                return helper(index + 1, k + direction*(-1), direction*(-1))
            return helper(index + 1, k + direction*(1), direction)
    return helper(1, 1, 1)  
...
def missing_digits(n):
    ...
    True
    """
    def helper(last, second_to_last):
        if last // 10 == 0:
            if last - 1 == second_to_last:
                return 0
            elif last == second_to_last: return 0
            else:
                return last - second_to_last - 1
        else:
            return helper(n % 10, (n//10) % 10) + missing_digits(n//10)
    return helper(n,n)
...
def count_coins(change):
    ...
    return helper(change, 1) # This is the startig state
